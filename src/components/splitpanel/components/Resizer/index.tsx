...
export const Resizer = ({
  ...
  const { grabberSize, css, hoverCss } = {
    ...defaultResizerOptions,
    ...resizerOptions,
  };

  const classes = useMergeClasses(["Resizer", split, className]);
  const grabberSizeWithUnit = useMemo(
    () => getSizeWithUnit(grabberSize),
    [grabberSize]
  );
  ...
  const collapseButton = collapseOptions ? (
    ...
    >
      ...
      >
        ...
        >
          {isCollapsed
            ? collapseOptions.afterToggleButton
            : collapseOptions.beforeToggleButton}
        </ButtonWrapper>
      </Transition>
