...
const PaneRoot = styled.div<PaneRootProps>`
  ...
  flex-grow: 1;
  flex-shrink: 1;
  ${(props: any) => (props.$isVertical ? verticalCss : horizontalCss)}
  ${(props: any) =>
    props.$shouldAnimate &&
    `transition: flex-basis ${props.$timeout}ms ease-in-out`}
`;
...
const CollapseOverlay = styled.div<{ $timeout: number; $isCollapsed: boolean }>`
  ${(props: any) => props.$isCollapsed && coverCss}
  ${(props: any) =>
    props.$isCollapsed &&
    css`
      z-index: 1;
    `};
  opacity: ${(props: any) => (props.$isCollapsed ? 1 : 0)};
  transition: opacity ${(props: any) => props.$timeout}ms ease-in-out;
`;
